...
function splitDeliveryUnits(content) {
  const units = [];
...
function aggregateUnits(units, maxLength) {
  const chunks = [];
  let current = '';
...
function packMarkdownBlocks(content, maxLength) {
...
function splitLongFencedLine(opening, line, closing, maxLength) {
  const chunks = [];
...
function findTruncationBoundary(text, maxBytes) {
  let bestBoundary = -1;
  let bestSentence = -1;
  let bytes = 0;
