...
test('WeixinIlinkClient.getUpdates posts iLink payload with authorization', async () => {
  ...
  assert.match(calls[0].init.headers.Authorization, /^Bearer bot-token$/);
...
test('WeixinIlinkClient.sendMessage and getConfig use Hermes-compatible payload fields', async () => {
  ...
  assert.equal(config.typing_ticket, 'typing-1');
  const sendBody = JSON.parse(calls[0].init.body);
  ...
  const configBody = JSON.parse(calls[1].init.body);
